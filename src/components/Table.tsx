    ...
    <div className="relative overflow-x-auto">
      ...
      <div className={`overflow-y-auto bg-[#F3F3F3]`}>
        <table className="min-w-full">
          ...
          <thead className="sticky top-0 z-10 h-[56px] border-b-2 bg-white text-[#B0B0B0]">
            <tr>
              {columns.map((column, index) => (
                <th
                  key={index}
                  style={{ width: column.width }}
                  className={`px-4 py-2 text-left text-[16px] font-semibold text-[#B0B0B0] 2xl:text-[20px]`}
                >
                  {column.header}
                </th>
              ))}
                ...
                >
                  {columns.map((column, index) => (
                    <td
                      key={index}
                      style={{ width: column.width }}
                      className={`h-[68px] px-4 py-2 text-[16px] font-[500] text-[#5B5B5B] 2xl:text-[20px] ${column.accessorKey === "type" && "text-center"} `}
                    >
                      {column.accessorKey === "action"
                        ? renderAction && renderAction(d)
                        : column.accessorKey === "topic" && renderTopic
                          ? renderTopic(d)
                          : column.accessorKey === "additionalInfo" &&
                              renderAdditionalInfo
                            ? renderAdditionalInfo(d)
                            : column.accessorKey === "financialStatus" &&
                                renderFinancialStatus
                              ...
                              : column.accessorKey === "status" && renderStatus
                                ...
                                : column.accessorKey === "name" &&
                                    renderSchoolName
                                  ? renderSchoolName(d)
                                  : column.accessorKey === "qualifications" &&
                                      renderQualificationName
