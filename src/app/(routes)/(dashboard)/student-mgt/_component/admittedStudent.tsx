...
interface AdmittedStudentProps {
  studentApi: any;
}

const AdmittedStudent = ({ studentApi }: AdmittedStudentProps) => {
      ...
      <div className="h-full w-full bg-white px-[8px]">
        <Table
          ...
          renderStatus={(item: any) => (
            <div className="">
              <p
                className={`${item.status === "Pending" && "bg-[#E6E6E6] px-[16px] py-[8px] text-center text-[#1E1E1E]"} ${item.status === "Approved" && "text-[#00BF00]"} ${(item.status === "Active" || item.status === "active") && "bg-[#E3EFED] px-[16px] py-[8px] text-center text-[#00473E]"} ${item.status === "Ended" && "text-[#ED1000]"}`}
              >
                {item.status}
              </p>
            </div>
          ...
          renderFinancialStatus={(item: any) => (
            <div className="">
              <p
                className={`${item.financialStatus === "Disabled" && "bg-[#FEF0F0] px-[16px] py-[8px] text-center text-[#9D1217]"}`}
              >
          ...
          renderAvatarImage={(item) => {
            ...
            const avatarSrc =
              item.avatar && item.avatar !== ""
                ? item.avatar
                : avatars[item.id % avatars.length];
