...
import { useSchoolByIdData } from '@/hooks/useSchool'
...
const SchoolId = () => {
  ...
  const queryClient = useQueryClient();
     const { mutate, isPending } = useMutation({
        mutationFn: (values: z.infer<typeof schoolFormSchema>) => updateSchoolMutationFn(schooData.id, values),
      });
      ...
      const form = useForm<z.infer<typeof schoolFormSchema>>({
        ...
        defaultValues: {
          name: schooData?.name || "",
        description: schooData?.description || "",
        },
      });
